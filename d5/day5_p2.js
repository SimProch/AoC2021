...
function dayFive(data) {
    ...
    function fillGrid() {
        rangeData.forEach(range => {
            ...
            const toY = range.toY;
            if (fromX === toX) fillVertical();
            if (fromY === toY) fillHorizontal();
            if (fromX !== toX && fromY !== toY) fillDiagonal();
            
            function fillDiagonal() {
                const decrementX = fromX > toX;
                const decrementY = fromY > toY;
                let x = fromX;
                let y = fromY;
                while (x !== toX) {
                    grid[y][x]++;
